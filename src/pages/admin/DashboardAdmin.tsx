...
const ModalEditarUsuario: React.FC<({
  ...
  export default function DashboardAdmin() {
    ...
    const [modalAlterarPapel, setModalAlterarPapel] = useState<any | null>(
      null,
    );
  ...
  async function handleCriarUsuario(dto: any) {
    setEnviando(true);
    try {
      await api.post("/usuarios/admin/criar", dto);
  ...
  async function handleExcluirDemanda() {
    ...
    try {
      await api.delete(`/demandas/${modalExcluirDemanda.id}`);
      setDemandas((prev) =>
